...
void get_pressure(const Vector<double>& x, double& pressure)
{
 ...
 // Default just use a constant force
  ...
  //pressure = p_mag*(r*r-0.5);
  pressure = p_mag*((1.0/(sqrt(1.0-(r*r))))-2.0);
 //pressure = p_mag;
}
  ...
  Vector<double>& u, const Vector<double>& u_exact, double& error, double& norm)
{
 ...
 error = pow((-x[1]*u[1] + x[0]*u[2])/r,2);
 //norm = (r*r-0.5);
 //norm = (1.0/(sqrt(1.0-(r*r))))-2.0;
 norm  = pow(( x[0]*u[1] + x[1]*u[2])/r,2);
}
...
// Set the free boundary conditions
bool found_it=false;
unsigned nbound = Outer_boundary1 + 1;
  ...
  for (unsigned inod=0; inod<num_nod; inod++)
   {
    ...
    if (nod_pt->x(0) == 1.0)
     {
      found_it=true;
      oomph_info << "Found correct boundary\n";
      oomph_info << "x = ("<<nod_pt->x(0)<<" "<<nod_pt->x(1)<<") \n";
...
if (!found_it)
 {
  oomph_info << "Not found node!" << std::endl;
  abort();
 }

// // Set the clamped boundary conditions
// unsigned nbound = Outer_boundary1 + 1;
// for (unsigned b = 0; b<nbound; b++)
//  {
//   // Local block
...
sprintf(filename,"RESLT/soln_fluid_low_p_0.09_%i.dat",Doc_info.number());
some_file.open(filename);
Bulk_mesh_pt->output(some_file,npts); 
some_file << "TEXT X = 22, Y = 92, CS=FRAME T = \"" 
       << comment << "\"\n";
...
// //double error,norm,dummy_error,zero_norm;
 ...
 if (zero_norm != 0.0)
 {
 //Trace_file << TestSoln::element_area << " " << zero_norm << std::endl;
 // output sq of error
 some_file.open("RESLT/error_fluid_low_low_p_0.005.txt", ofstream::out | ofstream::app);
 some_file << TestSoln::p_mag << " " << sqrt(dummy_error/zero_norm) << '\n';
 some_file.close();
 // some_file.open("RESLT/error_old.txt", ofstream::out | ofstream::app);
 // some_file << TestSoln::p_mag << " " << sqrt(sqrt(dummy_error)) << '\n';
 // some_file.close();
 }
 some_file.open("RESLT/error_old_fluid_low_low_p_0.005.txt", ofstream::out | ofstream::app);
 some_file << TestSoln::p_mag << " " << sqrt(dummy_error) << '\n';
 some_file.close();
 // some_file.open("RESLT/error_old_sq_sq.txt", ofstream::out | ofstream::app);
 // some_file << TestSoln::p_mag << " " << dummy_error << '\n';
 // some_file.close();
 
 
 //oomph_info << "Norm of computed solution: " << zero_norm
 //<< std::endl;
 ...
 oomph_info << "w in the middle: " << std::setprecision(15) << u_0[0] << std::endl;
  ...
  // doc u_0 against element area
 some_file.open("RESLT/element_area.txt", ofstream::out | ofstream::app);
 some_file << TestSoln::element_area
            << " " << u_0[0] << '\n';
 some_file.close();
...
int main(int argc, char **argv)
{
 ...
 // // Element area minimum
 // double element_area_min = 0.001;

 // // Element area step
 // double element_area_step = 0.005;
 ...
 //   // Now do simple loop until target element area and solve
 // while(TestSoln::element_area>element_area_min)
 // {
 ...
 while(TestSoln::p_mag<p_max)
  ...
  oomph_info << "Element area (" << TestSoln::element_area << ")" << std::endl;
  ...
  // // Decrease the element area
  // TestSoln::element_area -= element_area_step;
